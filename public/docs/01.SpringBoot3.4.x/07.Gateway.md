...
## 예제 코드
```java
// GatewayApplication.java — API 게이트웨이의 진입점이에요
...
public class GatewayApplication {
    ...
    @Bean
    public RouteLocator customRoutes(RouteLocatorBuilder builder) {   // ← 라우팅 규칙을 자바 코드로 정의해요
        return builder.routes()
            .route("user-service", r -> r                             // ← "/users/**" 경로를 USER-SERVICE로 라우팅
                .path("/users/**")
                .filters(f -> f
                    .addRequestHeader("X-Gateway-Source", "cloud-gw")  // ← 요청 헤더를 추가해요
                    .circuitBreaker(c -> c                             // ← 장애 시 fallback URI로 전환
                        .setName("userCircuit")
                        .setFallbackUri("forward:/fallback/users")))
                .uri("lb://USER-SERVICE"))                             // ← 로드밸런싱 URI (Discovery Service 필요)
            .route("order-service", r -> r
                .path("/orders/**")
                .filters(f -> f
                    .retry(config -> config                            // ← SERVICE_UNAVAILABLE 시 최대 3회 재시도
                        .setRetries(3)
...
```
```java
// CustomGatewayFilter.java — 모든 요청에 대해 로깅하는 전역 필터예요
...
@Component
public class CustomGatewayFilter implements GlobalFilter, Ordered {  // ← 모든 라우트에 적용되는 전역 필터

    private static final Logger log = LoggerFactory.getLogger(CustomGatewayFilter.class); // ← 로거 선언

    @Override
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ServerHttpRequest request = exchange.getRequest();
        log.info("Incoming request: {} {}", request.getMethod(), request.getURI());  // ← 요청 시작 시 로그
        return chain.filter(exchange)                                                   // ← 다음 필터로 전달
            .then(Mono.fromRunnable(() -> {
                ServerHttpResponse response = exchange.getResponse();
                log.info("Response status: {}", response.getStatusCode());              // ← 응답 완료 시 로그
            }));
    }

    @Override
    public int getOrder() {
        return -1;  // ← 가장 높은 우선순위 (숫자가 작을수록 먼저 실행)
    }
}
```
## 주요 패턴
